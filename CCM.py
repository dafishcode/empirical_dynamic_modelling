...
#==========================================    
def ccm_cellstack(coord_l, data_l, thresh):
#==========================================
    
    """
    This function groups all cells together across datasets above some threshold for directional CCM causation. 

    Inputs:
        coord_l (list): list of string of pre-CCM coordinate files to stack together
        data_l (list): list of strings of xmap files to stack together
        thresh (float): threshold for plotting

    Returns:
        dic (dict): dictionary containing all data
    ...
    dic = {'all neurons': []}
    name_l = 'neurons driving seizure', 'seizure driving neurons'
    [dic.update({name: [[],[]]}) for name in name_l] #list of list for outputs

    for fish in range(len(data_l)): #loop through each fish

        #LOAD
        coord_lab = np.load(coord_l[fish], allow_pickle=True).item()['coord'] #load coordinates with labels
        coord = coord_lab[:,:3].astype('float').astype('int') #convert to int 

        data = h5py.File(data_l[fish])
        ccm = np.array(data['ccm']) 
        c_to_sz = ccm[1:,0] #cells that drive the seizure
        sz_to_c = ccm[0,1:] #cells that are driven by the seizure

        #APPEND
        ccm_l = c_to_sz, sz_to_c
        #loop through each datatype and put into a list
        for i,name in enumerate(name_l):
            ind = ccm_l[i] > thresh
            dic[name][0] = np.append(dic[name][0],ccm_l[i][ind])
            dic[name][1] = np.append(dic[name][1],coord[ind])
            if i == 0:
                dic['all neurons'] = np.append(dic['all neurons'], coord)


    #Reshape coordinates from vector to 3d
    for i,name in enumerate((name_l)):
        dic[name][1] = np.reshape(dic[name][1], (np.int(dic[name][1].shape[0]/3),3))
    dic['all neurons'] = np.reshape(dic['all neurons'], (np.int(dic['all neurons'].shape[0]/3),3))
    return(dic)
